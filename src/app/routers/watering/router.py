...
@watering_router.patch(
    "/{watering_id}", status_code=status.HTTP_200_OK, response_model=WateringBase
)
async def update_item(
    ...
    return watering_crud.update(
        db=db, db_obj=watering_crud.get(db=db, obj_id=watering_id), obj_in=watering_data
    )
